...
atom_types: std.AutoHashMapUnmanaged(Atom.Index, Wasm.FunctionType.Index) = .empty,
...
pub const FunctionImportIndex = enum(u32) {
    ...
    fn ptr(index: FunctionImportIndex, zo: *const ZigObject) *Wasm.FunctionImport {
        return &zo.function_imports.items[@intFromEnum(index)];
    }
};
...
pub const FunctionIndex = enum(u32) {
    ...
    fn @"type"(index: FunctionIndex, zo: *const ZigObject) Wasm.FunctionType.Index {
        return zo.functions.items[@intFromEnum(index)];
    }
};
...
/// Index into `segment_info`.
pub const SegmentIndex = enum(u32) {
    _,

    fn ptr(i: @This(), zo: *const ZigObject) *Wasm.NamedSegment {
        return &zo.segment_info.items[@intFromEnum(i)];
    }
};
...
fn createStackPointer(zo: *ZigObject, wasm: *Wasm) !void {
    ...
    sym.pointee = .{ .global_import_zo = @enumFromInt(zo.global_imports.items.len) };
...
pub fn deinit(zig_object: *ZigObject, wasm: *Wasm) void {
    const gpa = wasm.base.comp.gpa;
    {
        ...
        while (it.next()) |nav_info| nav_info.exports.deinit(gpa);
    }
...
fn allocateSymbol(zo: *ZigObject, gpa: std.mem.Allocator) error{OutOfMemory}!Symbol.Index {
    ...
    zo.symbol(index).* = .{
        ...
        .pointee = undefined, // will be set during atom parsing
...
pub fn getErrorTableSymbol(zig_object: *ZigObject, wasm: *Wasm, pt: Zcu.PerThread) !Symbol.Index {
    if (zig_object.error_table_symbol) |s| return s;
...
fn populateErrorNameTable(zig_object: *ZigObject, wasm: *Wasm, tid: Zcu.PerThread.Id) !void {
    const error_table_symbol = zig_object.error_table_symbol orelse return;
    const gpa = wasm.base.comp.gpa;
    const atom_index = wasm.symbol_atom.get(.{ .file = .zig_object, .index = error_table_symbol }).?;
...
pub fn addOrUpdateImport(
    ...
    if (func_type_index) |ty_index| {
        ...
        sym.pointee.function_import_zo.ptr(zo).* = .{
...
pub fn getGlobalSymbol(zig_object: *ZigObject, gpa: std.mem.Allocator, name_index: Wasm.String) !Symbol.Index {
    ...
    zig_object.symbol(sym_index).* = .{
        ...
        .pointee = undefined, // index to type will be set after merging symbols
...
pub fn deleteExport(
    ...
    if (nav_info.@"export"(zig_object, name_interned)) |sym_index| {
        ...
        assert(wasm.symbol_atom.swapRemove(.{ .file = .zig_object, .index = sym_index }));
...
pub fn storeDeclType(
    zig_object: *ZigObject,
    gpa: std.mem.Allocator,
    nav_index: InternPool.Nav.Index,
    index: Wasm.FunctionType.Index,
